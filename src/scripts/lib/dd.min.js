...
(() => {
	...
	class t {
		constructor(e, t) {
			(this.ele = e),
				(this._settings = {
					byJson: { data: null, selectedIndex: 0, name: null, size: 0, multiple: !1, width: 250 },
					mainCss: "ms-dd",
					rowHeight: null,
					visibleRows: null,
					showIcon: !0,
		...
		_init() {
			this._makeLayout(),
				...
				this.ele.size > 1 && (this._makeUiAsList(!0, this.ele.size), this._scrollToItem()),
		...
		_checkDataSettings() {
			...
			(t.mainCss = e?.mainCss || t.mainCss),
				...
				(t.showPlusItemCounter = e?.showPlusItemCounter || t.showPlusItemCounter),
		...
		_showHideFilterBox(e = !0) {
			e
				? (this._show(this._wrapper.filterHolder),
				  ...
				  "false" === this._settings.showFilterAlways.toString() && this._hide(this._wrapper.headerA))
		...
		_makeHeader() {
			...
			return (
				...
				"left" !== this._settings.imagePosition && s.classList.add(e.imageRight),
		...
		_makeChildren(e = null) {
			...
			this._wrapper.listOfItems
				...
				: ((t = this._createEle("ul", { className: s.listOfItems, zIndex: this._settings.zIndex })),
			...
			for (let e = 0; e < h; e++) {
				let i = a[e],
					...
					o = this._createRow(d);
				if (
					("" !== h.className && (o.className = o.className + " " + h.className),
					...
					for (let i = 0; i < t; i++) {
						let t = n(e[i], i),
							...
							h = t.itemObj,
							d = this._createRow(h);
						"" !== a.className && (d.className = d.className + " " + a.className),
							"" !== a.internalStyle && (d.style = a.internalStyle),
							(d.index = a.index),
							d.setAttribute("data-ms-index", a.index),
							h.isSelected && this._setSelectedByItem(d, !0),
							r(d, h),
							"left" !== this._settings.imagePosition && d.classList.add(s.imageRight),
							l.appendChild(d);
					}
					o.appendChild(l);
				}
				d.isOptGroup || r(o, d),
					"left" !== this._settings.imagePosition && o.classList.add(s.imageRight),
					t.appendChild(o),
		...
		_makeLayout() {
			...
			let r = this._createEle("div", { className: "more", style: "display:none" });
			...
			s.appendChild(u),
				this._bindEvents(this._wrapper.holder, "focus", (t) => {
					this._isList
						...
						: (e && (e.close(null), (e = null)), this._bindDocumentEvents(null, !0, !0), (e = this)),
				...
				this._bindEvents(n, "invalid", (e) => {
					e.target.setCustomValidity(""),
						e.target.validity.valid || e.target.setCustomValidity(this._settings.errorMessage);
				}),
		...
		_createRow(e) {
			...
			if ((a.appendChild(r), null !== e.img.src)) {
				let t = this._createEle("img", { className: e.img.css, src: e.img.src });
				l.appendChild(t);
			}
			if (null === e.img.src && "" !== e.imageCss.replace(/\s/g, "")) {
				let t = this._createEle("span", { className: e.img.css + " " + e.imageCss }, "&nbsp;");
				l.appendChild(t);
			}
			if (null !== e.desc.text) {
		...
		_removeOldSelected() {
			...
			for (let t = 0; t < e.length; t++)
				e[t].classList.remove(this._css.itemSelected),
					this._isMultiple && "true" === this._enableCheckbox.toString() && (e[t]._refCheckbox.checked = !1);
		}
		...
		_setSelectedByItemToggle(e, t) {
			let s = !e.checked,
				i = t.index;
			s
				? (t.classList.add(this._css.itemSelected), (this.ele.options[i].selected = !0))
				: (t.classList.remove(this._css.itemSelected), (this.ele.options[i].selected = !1)),
				this.updateUiAndValue();
		}
		_setSelectedByItem(e, t = !1, s = !0) {
			...
			"true" === this._enableCheckbox.toString() && e?._refCheckbox && (e._refCheckbox.checked = !0),
				!1 === this._isFirstTime && (this._fireLocalEventIfExist("change"), this._fireEventIfExist("change")),
				(this._isFirstTime = !1);
		}
		...
		_updateHeaderUI(e = null, t = null) {
			...
			if (
				("true" === this._settings.showPlusItemCounter.toString() &&
					...
					s.ui.length > 1 &&
					(l.innerHTML =
						l.innerHTML + `<span class="${this._css.headerCounter}">&nbsp; (+${s.ui.length - 1})</span>`),
		...
		_getDataAndUI(e = null) {
			let t,
				...
				d = function (e) {
					let t = l.options[e],
						s = e;
					return { option: t, data: a._parseOption(t), index: s, ui: a._findElementByIndexProp(s) };
				};
			if (null !== e) (i = d(e)), (n = i.option), (t = i.data), (r = i.index), (s = i.ui);
				...
				for (let t = 0; t < s.length; t++)
					(i = d(s[t].index)), e.push(i.data), l.push(i.option), a.push(i.index), o.push(i.ui);
				(t = e), (n = l), (r = a), (s = o), (h = !0);
			} else
		...
		updateUiAndValue(e = null) {
			...
			if (
				...
				this._isMultiple && "[]" === s.name.substr(s.name.length - 2, s.name.length))
			) {
				this._wrapper.moreValueBox.innerHTML = "";
				for (let e = 1; e < t.data.length; e++) {
					let i = this._createEle("input", { type: "hidden", name: s.name, value: t.data[e].value });
		...
		_createEle(e, t, s) {
			...
			if (t) for (let e in t) "style" === e ? (i.style.cssText = t[e]) : (i[e] = t[e]);
		...
		_bindDocumentEvents(e, t = !0, s = !0) {
			this._unbindDocumentEvents(),
				...
				!0 === t && this._bindEvents(document, "mouseup", this._onDocumentClick),
				!0 === s &&
		...
		_unbindDocumentEvents() {
			null !== this._onDocumentClick && this._unbindEvents(document, "mouseup", this._onDocumentClick),
				null !== this._onDocumentKeyDown && this._unbindEvents(document, "keydown", this._onDocumentKeyDown),
				null !== this._onDocumentKeyUp && this._unbindEvents(document, "keyup", this._onDocumentKeyUp),
				(this._onDocumentClick = null),
				(this._onDocumentKeyDown = null),
				(this._onDocumentKeyUp = null);
		}
		...
		_has_handler(e) {
			...
			if ("undefined" != typeof jQuery) {
				let s,
					i = jQuery(this.ele);
				(s = "function" == typeof jQuery?._data ? jQuery?._data(this.ele, "events") : i.data("events")),
		...
		previous() {
			...
			if ((s.length, (e = this._isFilterApplied ? this._nexPrevCounter : this.selectedIndex), s.length > 0)) {
				let i = (function (i) {
					...
					for (; l > 0; l--) {
						let e = l - 1;
						if (((e = e >= 0 ? e : 0), t._nexPrevCounter--, !s[e].classList.contains(t._css.itemDisabled)))
							return s[e];
					}
					return null;
				})();
		...
		close(e) {
			...
			this.disabled ||
				...
				(this._wrapper.filterInput.value = ""),
				this._wrapper.filterInput.blur(),
				"false" === this._settings.showFilterAlways.toString() && this._showHideFilterBox(!1),
				this._applyFilters(),
				this._unbindDocumentEvents(),
				this._updateHeaderUI(),
				this.ele.length !== this._getAllEle(`li.${this._css.item}`, this._wrapper.listOfItems).length &&
		...
		set selectedIndex(e) {
			...
			if (e < this.length && !this._isArray(e)) s(e);
				...
				for (let t = 0, s = e.length; t < s; t++)
					e[t] < this.length && -1 !== e[t] && this._setSelectedByOptionItem(this.ele.options[e[t]], t > 0);
		...
		set options(e) {
			e instanceof HTMLOptionElement
				...
				: "number" == typeof e && ((this.ele.length = e), this._makeChildren(), this.updateUiAndValue());
		}
		get optionsUI() {
			return (
				this._cacheEle.allItems,
				(this._cacheEle.allItems = this._getAllEle(`li.${this._css.item}`, this._wrapper.listOfItems))
			);
		}
		get length() {
		...
		set disabled(e) {
			e
				? (this.ele.setAttribute("disabled", ""), this._wrapper.holder.classList.add(this._css.wrapperDisabled))
		...
		set multiple(e) {
			e
				? (this.ele.setAttribute("multiple", ""), (this._enableCheckbox = this._settings.enableCheckbox))
				: this.ele.removeAttribute("multiple"),
				(this._isMultiple = e),
				e || ((this.selectedIndex = this.ele.selectedIndex), (this._enableCheckbox = !1)),
				this._makeChildren();
		}
		get name() {
	...
	customElements.define("ms-dropdown-ligthhudate", i, { extends: "select" }),
		(window.MsDropdown = s),
		navigator.vendor &&
			...
			s.make("select[is='ms-dropdown-ligthhudate']");
})();
